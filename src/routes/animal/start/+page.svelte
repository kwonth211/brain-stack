...
<svelte:head>
	<title>동물 퀴즈 - dual-brain</title>
	<meta name="description" content="동물 퀴즈를 풀어보세요, 많이 풀수록 랭킹에 올라갑니다." />
</svelte:head>
...
<div in:fade class="container">
	...
	<div class="title-container">
		<div class="title">
			동물 퀴즈
			<div class="sub-title">동물 박사는 누구일까요?</div>
		</div>
	</div>
		...
		{#if !allQuizResolved}
			<Button
				...
				onclick={async () => {
					...
					if (!quiz) {
						goto(`/quiz/complete?category=14`);
						return;
					}
					goto(`/quiz/${quiz.id}?category=14`);
				}}>시작하기</Button
			>
		{/if}
	</div>
	...
	{#if CompleteModalOpen}
		...
		>
			<QuizCompleteNormal title={`🎉 대단해요 🎉<br/>동물 문제를 모두 해결했어요`} />
		</Modal>
	{/if}
