...
<div in:fade class="container">
	...
	<div class="title-container">
		<div class="title">
			퓨쳐위즈 퀴즈
			<div class="sub-title">퓨쳐위즈에서 가장 센스있는 동료는?</div>
		</div>
	</div>
		...
		{#if !allQuizResolved}
			<Button
				...
				onclick={async () => {
					...
					if (!quiz) {
						goto(`/quiz/complete?category=999`);
						return;
					}
					goto(`/quiz/${quiz.id}?category=999`);
				}}>시작하기</Button
			>
		{/if}
	</div>
	...
	{#if CompleteModalOpen}
		...
		>
			<QuizCompleteNormal title={`🎉 대단해요 🎉<br/>퓨쳐위즈 문제를 모두 해결했어요`} />
		</Modal>
	{/if}
